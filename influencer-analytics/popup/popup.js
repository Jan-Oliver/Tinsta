document.addEventListener('DOMContentLoaded', function() {
  ...
  chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
    ...
    const isSupportedSite = currentTab.url.includes('tiktok.com') || 
                           (currentTab.url.includes('instagram.com') && currentTab.url.includes('/reels'));
    
    if (isSupportedSite) {
      document.getElementById('not-supported-message').style.display = 'none';
      document.getElementById('supported-content').style.display = 'block';
    } else {
      document.getElementById('not-supported-message').style.display = 'block';
      document.getElementById('supported-content').style.display = 'none';
    }
  });

  // Set up open panel button
  document.getElementById('open-panel-button').addEventListener('click', function() {
    chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
